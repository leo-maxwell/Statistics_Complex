...
\section{独立 (Independent)}
定义：对于两个分别满足概率密度函数$f_X$和$f_Y$的随机变量$X$和$Y$而言，如果它们的联合概率密度函数$f_{X,Y}=f_Xf_Y$，则称这两个随机变量是相互独立的。

直观上而言，独立性保证了两个随机变量之间不会互相影响，即上海的一家酒吧里掷出的骰子的点数和波士顿今日的平均气温无关。独立是一个很强的条件。
...
\begin{description}
	...
	\item [连续型：]
	$\mathrm E[X]=\int_{\mathbb{R}}xf(x)\mathrm dx$
\end{description}
...
关于随机变量的期望，有一个重要的定理：（证明请见\ref{LUTOS-proof}）
...
\end{description}
\section{不相关 (Uncorrelated)}
\label{def:uncorrelated}
定义：对两个随机变量$X,Y$而言，如果$\mathrm{Cov}(X,Y)=0$，则称$X,Y$是互不相关的。

不相关是一个比独立更弱的条件，因为相互独立的变量一定互不相关，反之不然（证明见\ref{proof:uncorrelated}）。这里的相关仅仅指线性意义上的相关，例如$X$和$aX+b$一定相关，但是$X$和$X^2$互不相关，但很显然它们不相互独立。
...
\subsection{不相关和独立的关系}
\label{proof:uncorrelated}
首先证明互相独立一定互不相关：
\begin{align*}
	\mathrm{Cov}(X, Y)&=\mathrm E[XY]-\mathrm E[X]\mathrm E[Y] \\
	&=\int_\mathbb{R}xyf_{XY}(xy)\mathrm dx\mathrm dy-\int_\mathbb{R}xf_X(x)\mathrm dx\int_\mathbb{R}yf_Y(y)\mathrm dy \\
	&=\int_\mathbb{R}xyf_X(x)f_Y(y)\mathrm dx\mathrm dy-\int_\mathbb{R}xf_X(x)\mathrm dx\int_\mathbb{R}yf_Y(y)\mathrm dy \\
	&=0
\end{align*}

然后证明存在不互相独立但是互不相关的例子：

以投掷七面骰为例，令随机变量$X'$表示投掷出的点数，并令$X=X'-4, Y=X^2$，容易知道$X,Y$显然是不互相独立的：

\begin{center}
	\begin{tabular}{c c c c c c c c c}
\hline \vspace{-0.3cm}\\
	 $p$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\frac{1}{7}$ &$\mathrm E$ \\
	 \vspace{-0.3cm} \\
	 \hline $X$ &-3 &-2 &-1 &0 &1 &2 &3 &0 \\
	 $X^2$ &9 &4 &1 &0 &1 &4 &9 &4 \\
	 $X^3$ &-27 &-8 &-1 &0 &1 &8 &27 &0 \\
	\hline
\end{tabular}
\end{center}
...
它们的协方差：
\begin{align*}
	\mathrm {Cov}(X, Y)&=\mathrm E[X^3]-\mathrm E[X]\mathrm E[X^2] \\
	&=0
\end{align*}
