...
%\SetWatermarkText{Maxwell General Learning System Academic\\\\
%Confidential\\\\ 麦克斯韦通用高等习得系统\\\\ 不公开}
...
若两个随机变量X和Y相互独立，则$E[(X-E(X))(Y-E(Y))]=0$，因而若上述数学期望不为零，则$X$和$Y$必不是相互独立的，亦即它们之间存在着一定的关系。
...
\begin{itemize}
	...
	\item $Cov(aX,bY)=abCov(X,Y),(a,b\in \mathbb{C});$(线性)
	\item $Cov(X_1+X_2,Y)=Cov(X_1,Y)+Cov(X_2,Y)$
	\item $Cov(X+a,Y+b)=Cov(X,Y)$
	\item $Cov(X,X)=Var(X)$(零自协方差)
\end{itemize}
